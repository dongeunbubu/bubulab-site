...
(function(){
  ...
  W2['step-guide']=function(el){
   ...
   function render(){
    ...
    el.innerHTML='<div class="cxw-sg-head"><span class="cxw-sg-n">STEP '+(idx+1)+' / '+steps.length+'</span><span class="cxw-sg-dots">'+dots+'</span></div>'
     +'<div class="cxw-sg-stage" style="aspect-ratio:'+ar+'">'+(s.img?lazyImg(s.img,s.alt||s.title||s.cap,'cxw-sg-img'):'<div class="cxw-sg-noimg">이미지 준비 중</div>')+badges(s)+'</div>'
 ...
 function showView(mode){
  var ex=d.getElementById('cxExplore'),rd=d.getElementById('cxReader'),tl=d.getElementById('cxTool'),pg=d.getElementById('cxProg');
  if(ex)ex.hidden=(mode!=='explore');if(rd)rd.hidden=(mode!=='reader');if(tl)tl.hidden=(mode!=='tool');
  if(pg)pg.classList.toggle('on',mode==='reader');
  try{scrollTo({top:0,behavior:'auto'});}catch(e){try{scrollTo(0,0);}catch(e2){}}
 }
 function route(){
  var p=parseHash();
  if(p.mode==='reader'&&isColumn(p.slug)){_route='reader:'+p.slug;showView('reader');renderReader(p.slug);}
  else if(p.mode==='reader'&&isTool(p.slug)){_route='tool:'+p.slug;showView('tool');renderTool(p.slug);}
  else{_route='explore';showView('explore');reveal(d.getElementById('cxExplore')||W,'.cx-rv');}
  updateProg();
 }
 ...
 function boot(){
  ...
  if(chips)chips.addEventListener('click',function(e){var b=e.target.closest('.cx-chip');if(!b||!chips.contains(b))return;[].slice.call(chips.querySelectorAll('.cx-chip')).forEach(function(x){x.classList.remove('on');x.setAttribute('aria-pressed','false');});b.classList.add('on');b.setAttribute('aria-pressed','true');ST.theme=b.getAttribute('data-theme')||'all';saveFilt();render();});
  var ftabs=d.getElementById('cxFuncTabs');
  if(ftabs)ftabs.addEventListener('click',function(e){var b=e.target.closest('.cx-functab');if(!b||!ftabs.contains(b))return;var fn=b.getAttribute('data-func')||'all';if(b.classList.contains('on')&&fn!=='all'){ST.func='all';}else{ST.func=fn;}[].slice.call(ftabs.querySelectorAll('.cx-functab')).forEach(function(x){var on=(x.getAttribute('data-func')||'all')===ST.func;x.classList.toggle('on',on);x.setAttribute('aria-pressed',on?'true':'false');});saveFilt();render();});
  (function(){if(chips){[].slice.call(chips.querySelectorAll('.cx-chip')).forEach(function(x){var on=(x.getAttribute('data-theme')||'all')===ST.theme;x.classList.toggle('on',on);x.setAttribute('aria-pressed',on?'true':'false');});}if(ftabs){[].slice.call(ftabs.querySelectorAll('.cx-functab')).forEach(function(x){var on=(x.getAttribute('data-func')||'all')===ST.func;x.classList.toggle('on',on);x.setAttribute('aria-pressed',on?'true':'false');});}})();
  ...
  var mbtn=d.getElementById('cxModeBtn');
  if(mbtn)mbtn.addEventListener('click',toggleMode);
  applyMode(getMode(),false);
  if(snap&&snap.length){useData(snap);}
  route();
  addEventListener('hashchange',route);
  if(window.fetch){fetch(LEG_RAW+'?cb='+Date.now(),{cache:'no-store'}).then(function(r){if(!r.ok)throw 0;return r.json();}).then(function(j){var it=(j&&j.items)||j;if(it&&it.length)useLegacy(it);}).catch(function(){});}
  if(window.fetch){
